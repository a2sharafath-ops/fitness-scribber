...
test('TC-005: future-effective assessment is not usable for an earlier session',()=>{const d=base();d.observations[0].effectiveAt='2026-09-08T10:00:00Z';assert.equal(resolveContext(d).facts.left.state,'missing')})
test('TC-004 TC-008: different units or protocols are not converted into equivalent findings',()=>{for(const patch of [{unit:'category'},{protocol:'different-method'}]){const d=base();Object.assign(d.observations[0],patch);assert.equal(resolveContext(d).facts.left.state,'missing')}})
test('TC-028 TC-044: unknown adult status and unanswered health checks remain required',()=>{const d=base();d.adultConfirmed=undefined;d.healthChange=undefined;const r=resolveContext(d);assert(r.reasons.some(x=>x.code==='adult_scope_unconfirmed'));assert(r.reasons.some(x=>x.code==='health_check_required'));assert.equal(r.state,'information_required')})
