...
test('gateway passes only verified actor and loaded source token to storage',async()=>{
 const calls=[]
 const serviceClient={rpc:async(name,payload)=>{calls.push({name,payload});return name==='pooling_decision_input'?{data:{sourceToken:'synthetic-token'}}:{data:{decisionId:1}}}}
 ...
 assert.equal(receipt.decisionId,1);assert.equal(calls[1].payload.verified_actor,'coach');assert.equal(calls[1].payload.expected_source_token,'synthetic-token')
})
