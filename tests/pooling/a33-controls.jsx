...
export function Controls(){
 ...
 const control=async action=>{const r=await fetch('/__a33/control',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({action})});const data=await r.json();if(!r.ok)throw Error(data.error||'Verification failed');setStatus(data);return data}
 ...
 const loadPending=async()=>{const rows=await readPendingOperations(workspace.clientId);setPending(rows);return rows.map(r=>({kind:r.kind,key:r.request.operationKey,scope:r.scope}))}
 ...
 <button onClick={()=>run(async()=>{const {data:{session}}=await supabase.auth.getSession();const r=await fetch('/__a33/expired-session',{method:'POST',headers:{Authorization:'Bearer '+session.access_token}});const old=await r.json();if(!r.ok)throw Error(old.error);const {data,error}=await supabase.auth.setSession(old);if(error)throw error;return {expiredProviderSessionRestored:true,actor:data.user.id,expiresAt:data.session.expires_at}})}>Restore expired provider session</button>{' '}
 ...
 {pending.map(r=><div key={r.request.operationKey}><p>{r.kind} · {r.scope}</p><button onClick={()=>run(async()=>{const result=await retryPendingOperation(r);await loadPending();return result})}>Reconcile {r.kind}</button></div>)}
...
// Keep one root when the private control page is updated during a long expiry
// test. HMR must not mount competing authentication listeners on the same node.
