...
test('a confirmed purpose answer cannot replace server scope and consent admission',()=>{for(const authority of [false,null,undefined]){const input=bundle();input.purposeAuthority=authority;assert.equal(buildSourceSnapshot(input).contextInput.purposePermitted,false)}})
