...
import PoolingWorkoutPreview from './PoolingWorkoutPreview'
...
export default function PoolingApprovalReview({clientId,context,drafts,workspace,online,onRefresh,blockedReason=''}){
 ...
 return <section className="card" aria-labelledby="pool-exact-title"><h2 id="pool-exact-title" tabIndex={-1}>Exact-revision review and assignment</h2>
  ...
  {!drafts.length&&<p className="pooling-next-step">{blockedReason||'No saved drafts yet. Use Prepare workout above to generate from this client’s reviewed inputs. Its Validate and Approve buttons will appear here.'}</p>}
  ...
  {drafts.map(row=>{
   ...
   return <details key={key} open={!assignments.some(a=>a.draftId===row.id)&&!drafts.some(child=>child.parent_id===row.id)}><summary>{row.proposal.date} · draft {row.id || 'local'} · revision {row.revision}{assignments.some(a=>a.draftId===row.id)?' · assigned':''}</summary>
    ...
    <details><summary>Exact revision changes</summary><pre style={{whiteSpace:'pre-wrap',overflowWrap:'anywhere'}}>{JSON.stringify(diff,null,2)}</pre></details>
    ...
    {decision && <><p>Decision {decision.id} · {decision.result.completeness.replaceAll('_',' ')} · expires {decision.validUntil}</p><PoolingBudgetNotice result={decision.result}/><PoolingWorkoutPreview blocks={decision.result.blocks} catalogue={workspace.manifests?.find(m=>m.id===row.proposal.manifestId)?.document.catalogue||[]}/><details><summary>Decision evidence and required gaps</summary><pre style={{whiteSpace:'pre-wrap',overflowWrap:'anywhere'}}>{JSON.stringify({blocks:decision.result.blocks,gaps:decision.result.gaps,durationSeconds:decision.result.durationSeconds},null,2)}</pre></details></>}
