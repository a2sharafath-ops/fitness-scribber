...
export default function CoachWorkoutPage(){
 ...
 const assignments=data?.workspace.assignments||[],draft=currentCoachDraft(data?.pool.drafts||[],assignments)
 const decision=draft&&data?.workspace.decisions.find(item=>item.draftId===draft.id)
 ...
 const manifest=draft&&data?.workspace.manifests.find(item=>item.id===draft.proposal.manifestId)
 const latestSuggestion=useMemo(()=>[...(data?.suggestions||[])].sort((a,b)=>b.id-a.id)[0],[data?.suggestions])
 ...
 const pendingOperation=data?.pending?.[0]
 const block=draft?approvalBlock({row:draft,context:data?.pool.context,decision,drafts:data?.pool.drafts,assignments,online:!loading&&!error}):''
 ...
 async function validate(){await run('validate',async()=>{await decideDraft({clientId:id,draftId:draft.id,generation:data.pool.context.generation});setNotice('Checks complete. Review the workout and approve it when you are happy.')})}
 async function approve(){await run('approve',async()=>{const result=await approveDraft({clientId:id,draftId:draft.id,decisionId:decision.id,generation:data.pool.context.generation,operationKey:crypto.randomUUID()});setNotice(`Workout assigned to ${client.name}.`);setReviewed(false);setJustAssigned(true);return result})}
