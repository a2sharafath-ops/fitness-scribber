...
export default function CoachWorkoutPage(){
 ...
 return <div className="coach-page">
  ...
  {!loading&&!justAssigned&&(!draft||showSetup)&&<CoachWorkoutSetup clientId={id} client={client} onPrepared={async()=>{setShowSetup(false);setReviewed(false);setJustAssigned(false);setRevision(value=>value+1)}}/>}
  {!loading&&!justAssigned&&draft&&!readyDraft&&!showSetup&&<section className="coach-card coach-needs-attention"><div className="coach-section-heading"><span className="coach-step-number">!</span><div><h2>We need a little more information</h2><p>This draft has not been assigned.</p></div></div>{gaps.length?<ul>{gaps.map((gap,index)=><li key={index}>{coachGapText(gap)}</li>)}</ul>:<p>The available exercises do not yet cover every required workout section. Review the equipment, client abilities and confirmed movement needs.</p>}<button className="btn" onClick={()=>setShowSetup(true)}>Review workout details</button></section>}
  ...
  {!!assignments.length&&<section className="coach-card coach-action-card"><div><div className="coach-section-heading"><span className="coach-step-number">✓</span><div><h2>Assigned workouts</h2><p>{assignments.length} workout{assignments.length===1?' is':'s are'} saved for this client. Open the client summary to start a session or record results.</p></div></div></div><div className="coach-card-actions">{justAssigned&&<button className="btn ghost" onClick={()=>{setJustAssigned(false);setShowSetup(true)}}>Create another</button>}<Link className="btn" to={`/clients/${id}#workouts`}>Open workouts</Link></div></section>}
