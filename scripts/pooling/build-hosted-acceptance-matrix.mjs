...
// A32 adds evidence without erasing what A30 actually demonstrated. P still
// means the stated invariant at its named layer, never complete participant UAT.
const updates=[
 ['TC-001','P','completion-cases.test.js','Exact empty movement-source fixture stays missing, never assessed absent.'],
 ['TC-002','P','completion-cases.test.js','Exact untouched lifestyle and wellness defaults supply no confirmed observations. UI slider permutations are not independently certified.'],
 ['TC-005','P','completion-cases.test.js','Future-effective known input emits source_date_mismatch; future-recorded input is excluded from the historical cutoff.'],
 ['TC-012','P','completion-cases.test.js','Exact duplicate findings and three-candidate incremental-coverage fixture pass; uncovered need wins over duplicate coverage and pins.'],
 ['TC-019','P','completion-cases.test.js','Dedicated stale, future, wrong-variant and unsupported-method load-source fixtures all remain unresolved.'],
 ...
 ['TC-029','P','verification-hosted.mjs; verification-browser-receipt.mjs','Two real authenticated API sessions raced report versus approval; final Preview two-tab stale approval denied after another tab reported a concern.'],
 ['TC-032','P','verification-final-checks.mjs','Browser response dropped after real hosted commit for actual and draft: automatic same-key retry yielded exactly one row each.'],
 ['TC-033','P','verification-hosted.mjs; verification-browser-receipt.mjs','Same-owner API session races plus two actual Preview tabs prove stale-generation denial; no two-person UAT is claimed.'],
 ...
 ['CAT-006','P','completion-cases.test.js','Dedicated three-candidate incremental coverage and duplicate-signal fixture passes.'],
 ['CAT-014','P','completion-cases.test.js; ui.jsx','100-second deficit and safe alternatives rendered from engine output; optional omission does not make required over-budget work ready.'],
 ['CAT-015','P','verification-hosted.mjs; verification-browser-receipt.mjs','Hosted concurrent API race and actual two-tab stale-revision refusal pass.'],
 ['CAT-016','P','verification-final-checks.mjs','Actual browser dropped response after hosted commit replays one exact receipt without duplicate record.'],
 ['CAT-020','P','verification-final-checks.mjs','Simulated network outage in browser: pending actual, independent Stop, reconnect, exact retry and fresh-page retained history pass.'],
 ['R2-T02','P','completion-cases.test.js','Exact legacy lifestyle/daily defaults never become confirmed source authority.'],
 ['R2-T09','P','completion-cases.test.js','Fictional maximum 48 with 45/50 inventory cannot round to 50; exact admitted inventory is required.'],
 ['R2-T10','P','completion-cases.test.js; ui.jsx','Exact 100-second deficit is visible with alternatives; no hidden rest/dose trimming.'],
 ['R2-T13','P','numerical.test.js; ui.jsx; verification-hosted.mjs','Before/after full prescription and derived duration supplied by v2 hosted proposal; repetition/side/time table verified separately in isolated browser fixture.'],
 ...
 ['R3-T18','P','verification-hosted.mjs','New-health versus weekly approval race commits all or none; any winning assignments are stale/held and cannot start.'],
]
for(const [id,state,evidence,note] of updates){const row=indexed.get(id);assert(row,id);indexed.set(id,{...row,previousA30:{...row},status:state==='P'?'passed-engineering-invariant':'partial',evidence:evidence.split('; '),note})}
// A33 closes only the named engineering variations. Public reports retain the
// A30/A32 history and continue to distinguish human and domain acceptance.
const a33=[
 ['TC-023','a33-browser-server.mjs; a33-final-checks.mjs; template-import.test.js','Hosted-backed production UI copy-last, saved-template and same-coach cross-client imports each saved one unassigned draft; actuals/recipient-private notes stripped. Prior A32 manual/parser/tomorrow/bulk evidence retained; optional microphone/AI stays disabled.'],
 ['TC-040','a33-final-checks.mjs; a33-browser-server.mjs; remaining-browser.mjs; recovery-transport.test.js','Real provider-issued JWT expired naturally and was rejected; actual SDK refresh recovered its original pending request exactly once. Global sign-out, direct account switch, wrong-account retry and account change during held response passed. Real bounded browser storage/Web Locks proved separately in Chromium/WebKit; no physical-device certificate.'],
 ['TC-043','a33-browser-server.mjs; a33-final-checks.mjs; review.test.js','Hosted-backed native date/timezone form rejected disagreement; distinct Tokyo/Los Angeles drafts retain correct local dates and the same explicit instant. Old history remains intact.'],
 ['R2-T17','a33-browser-server.mjs; a33-final-checks.mjs','Actual hosted extension-review response was dropped after commit; browser same-key reconciliation yielded one acceptance and one separate unassigned child.'],
 ['R3-T05','a33-browser-server.mjs; a33-final-checks.mjs; weekly.test.js','Explicit travel reason, selected dates/zones and original history retained through actual UI saves. No catch-up assignment invented; the extra explicit Save has its own distinct key.'],
 ['R3-T08','a33-fixtures.mjs; a33-final-checks.mjs','Actual hosted fictional 2 kg history cannot propose unavailable 4 kg; exact target-session [2,4] kg confirmation admits the 2-to-4 proposal. Accepted child stays unassigned and original actuals unchanged. No clinical/exercise policy acceptance inferred.'],
]
for(const [id,evidence,note]of a33){const previous=indexed.get(id);assert(previous,id);indexed.set(id,{...previous,previousA32:{...previous},status:'passed-engineering-invariant',evidence:evidence.split('; '),note})}
