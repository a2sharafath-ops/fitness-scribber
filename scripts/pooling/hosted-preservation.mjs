...
const report={checkedAt:new Date().toISOString(),baselineTables:tables.length,baselineRows:Object.values(baseline).reduce((n,r)=>n+r.length,0),missingOrChangedBaselineTables:[...new Set(changes)],baselinePreserved:changes.length===0,addedBaselineRows,poolingRows,totalRetainedAddedRows:addedBaselineRows+poolingRows,withinApproved1000AtCheck:addedBaselineRows+poolingRows<=1000,storageObjects:Number(query(rt.bin,env,'select count(*) from storage.objects')),limits:'Retained-row count, not a lifetime count of managed transient tokens. Infrastructure singleton and 18 migration-ledger rows excluded. Identity inventory is private. No application/history deletion; Auth sign-out can remove run-created sessions/tokens.'}
...
assert.equal(report.baselinePreserved,true);assert.equal(report.withinApproved1000AtCheck,true);assert.equal(report.storageObjects,0)
