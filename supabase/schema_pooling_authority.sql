...
do $$ declare t text; begin
  foreach t in array array['pooling_manifests','pooling_decisions','pooling_assignments','pooling_execution_events'] loop
    ...
    execute format('revoke all on public.%I from public,authenticated',t);
  end loop;
end $$;
...
begin
 ...
 if found then
   if a.draft_id<>pooling_approve.draft_id or a.decision_id<>pooling_approve.decision_id or a.context_generation<>expected_generation then raise exception 'idempotency_conflict'; end if;
...
declare a public.pooling_assignments%rowtype; c public.clients%rowtype; ctx public.pooling_contexts%rowtype;
 ...
 last_kind text; event_id bigint; actor uuid:=auth.uid();
begin
 if actor is null then raise exception 'forbidden'; end if;
 ...
 if octet_length(event_payload::text)>65536 or event_payload ?| array['approval','targets','assigned','clearance','coachId'] then raise exception 'protected_field'; end if;
 ...
 elsif event_kind in ('pause','complete','actual') and (last_kind is null or last_kind not in ('start','resume','pause')) then
...
revoke all on function public.pooling_approve(text,bigint,bigint,bigint,text) from public;
revoke all on function public.pooling_execution(bigint,bigint,text,text,jsonb) from public;
